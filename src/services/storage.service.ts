...
export let clientes: Cliente[] = []; // Array para armazenar clientes
export let produtos: Produto[] = []; // Array para armazenar produtos
export let pedidos: Pedido[] = []; // Array para armazenar os pedidos
...
export function salvarDados(): void {
  ...
  const cabecalhoPedido = "id,cliente_id,total,formaPagamento,dataCriacao\n";
  const corpoPedido = pedidos
    .map(
      (pedidos) =>
        `${pedidos.id},${pedidos.cliente.id},${pedidos.total},${pedidos.formaPagamento},${pedidos.dataCriacao.toISOString()}`
    )
    .join("\n");
...
export function carregarDados(): void {
  ...
  if (fs.existsSync(PEDIDOS_CSV) && fs.existsSync(ITENS_PEDIDO_CSV)) {
    ...
    const novosPedidos = pedidosData
      ...
      .filter((line) => line)
      .map((line) => {
        const [id, cliente_id, total, formaPagamento, dataCriacaoStr] = line.split(","); // essas serão as informações que aparecerão no arquivo de pedidos // --- INÍCIO DA RECONSTRUÇÃO --- // Para cada linha de pedido, usamos o 'cliente_id' para encontrar o OBJETO Cliente completo no array 'clientes'.
