...
const DATA_DIR = path.join(__dirname, 'data'); // Pasta para organizar os arquivos
...
    console.log('\nSalvando dados nos arquivos CSV...');

    //um if else, для чека се о диетório existe.
...
        }
    
        const quantidadeStr = await askQuestion(`Quantidade de '${produtoEncontrado.nome}': `);
...
    
        const respostaUsuario = await askQuestion('Deseja adicionar outro produto? (s/n): ');// a resposta do usuário é armazenada na constante "respostaUsuario"
...
        const escolhaPagamento = await askQuestion('Digite qual será a forma de pagamento (1-4): ')
        
...
        const escolha = await askQuestion('Digite o número referente ao seu desejo: ')
